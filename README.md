...
외부망이 차단된 환경에서도 인터넷 없이 설치와 운영이 끝까지 되어야 한다는 제약을 전제로 배포 체계를 설계했습니다. 그래서 Python·JRE·Nginx 런타임과 오프라인 wheel을 전부 배포 zip에 담는 포터블 구조를 잡았고, 예측·학습을 담당하는 FastAPI AI 엔진과 React 프론트엔드, 공공 표준 프레임워크(eGovFrame) 백엔드를 Nginx 뒤에 3-티어로 묶었습니다.
