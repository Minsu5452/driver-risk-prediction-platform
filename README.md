...
운수종사자의 검사 결과와 과거 사고 이력으로 향후 교통사고 위험도를 예측하고, 결과를 화면에서 조회·해석·관리하는 웹 플랫폼입니다. 외부망 분리 환경의 Windows PC에 포터블 zip 하나로 설치되도록 만들었습니다.
...
- 위험도 예측: 업로드된 검사 결과를 도메인별 스태킹 앙상블과 시퀀스 LightGBM, 두 계열의 예측을 가중 블렌딩해 위험 점수(0~1)로 산출합니다. 시퀀스 모델 추론이 실패하면 스태킹 단독으로 폴백합니다.
...
- 운영 기능: 모델 버전 관리(활성 버전 전환·삭제·용량 조회), 중단된 학습의 자동 복구, 날짜별 회전 로깅, 시스템 사양에 맞춘 병렬도 자동 조정을 포함합니다.
...
- 검사 데이터 Excel은 전부 문자열로 읽습니다(`dtype=str`). 수치로 읽으면 식별자의 앞자리 0이 사라지는 사고가 나기 때문입니다. 사람 단위 식별자 정규화도 한 함수(`resolve_pk`)에만 두어 화면·이력·예측이 같은 기준을 쓰도록 했습니다.
